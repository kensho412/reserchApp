...
**すべてローカル / 無料 / OSS。** 有料クラウド LLM（OpenAI / Claude API 等）は一切使わない。LLM は Ollama、DB は SQLite、ベクトル検索は numpy（コンパイラ不要のローカル実装）、すべて手元で動く。
...
FastAPI Server
   │
Ollama  +  SQLite  +  numpy ベクトル  +  File Storage
```
...
```
reserchApp/
├── backend/                 FastAPI + Ollama + SQLite + numpy ベクトル
│   ├── app/
...
│   │   ├── vector.py        numpy ベクトルストア（embedding 類似検索・コンパイラ不要）
...
Python は **3.12** を想定（PyMuPDF の wheel 都合。3.14 はまだ未対応）。
...
4. embedding を作成し numpy ベクトルストア（DATA_DIR/vectors.pkl）に upsert
...
FastAPI 連携 / Ollama 要約・タグ候補 / 論文 Abstract 日本語訳 / SQLite + ベクトル保存。
